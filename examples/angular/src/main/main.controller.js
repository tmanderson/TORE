...
angular.module('TORE')
	.controller('MainController', function ($sce, $sceDelegate) {
		this.activeFeeds = [];
		this.entry = null;
		...
		this.toggleFeed = function(feed) {
			...
			_.each(feed.entries, function (entry) {
				...
				if(typeof entry.link === 'string') {
					entry.link = $sceDelegate.trustAs('resourceUrl', entry.link);
				}
				if(typeof entry.publishedDate === 'string') {
					entry.publishedDate = moment(entry.publishedDate).fromNow();
				}
			});
		};
	});
